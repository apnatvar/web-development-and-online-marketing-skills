...
> ⚠️ **`mj-accordion` and `mj-carousel` are documented here for reference but should not be used in production templates** (SKILL.md Rule 8). Both depend on CSS that the major clients strip: Outlook and older Gmail render an accordion **fully expanded**, defeating its purpose, and a carousel degrades to **the first image only**. The "falls back to…" notes below describe that degradation — read them as a warning, not reassurance. Prefer plain stacked sections over an accordion, and a single hero image over a carousel.
...
**`border` (MJML 5)**: Set once on `mj-social` to cascade to every `mj-social-element`'s `<img>` (useful for accessibility tools like Litmus/Email on Acid that flag icons without a border). An element-level `border` overrides the cascade for just that icon.
...
| title | string | — |
| icon-size | px / % | — |
| icon-position | left/right | left (MJML 5; had no default in v4) |
| font-size | px | 13px |
| color | CSS color | #000 |
| border | CSS border (MJML 5) | 0 |
| border-radius | px | 3px |
| padding | px / % | 4px |
...
**Injects an external font request.** `mj-navbar` unconditionally adds a Google Fonts `<link>` and `@import` for **Ubuntu** to the head, regardless of your `mj-font` / `mj-all` settings — an unrequested third-party request in every navbar email. `<mj-all font-family="...">` does not reach the hamburger label either; set `font-family` on each `mj-navbar-link` explicitly.

**Not idempotent.** Each compile emits a fresh random 16-hex checkbox ID, so two builds of the same source differ. Same for `mj-accordion` and `mj-carousel` — do not diff their output to detect real changes.
