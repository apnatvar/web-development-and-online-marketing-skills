...
**`gutter` (MJML 5)**: Sets equal spacing between every column in the section — an alternative to adding matching left/right padding to each `mj-column` by hand. Works with `mj-group` children too, and respects `direction="rtl"`.
...
**iOS stacking bug**: Even with `mj-group`, whitespace between tags can cause stacking. What prevents it is the `font-size:0px` MJML puts on the parent `<td>`. Note that in MJML 5 `--config.minify` no longer removes inter-tag whitespace (it collapses it to one space); add `--config.minifyOptions '{"collapseWhitespace":"all"}'` for the MJML 4 behavior.
